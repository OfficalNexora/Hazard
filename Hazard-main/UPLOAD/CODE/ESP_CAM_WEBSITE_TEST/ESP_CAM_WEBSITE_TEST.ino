...
#define HORIZONTAL_FLIP  true // Most mounts need both flips
#define CAMERA_XCLK_MHZ  8      // Dropping to 8MHz for maximum DMA buffer safety
...
String backend_url = "http://10.25.230.97:8000";
...
<body>
    ...
    <div class="card">
        <form action="/save" method="POST">
            ...
            <input type="text" name="backend" placeholder="http://10.25.230.97:8000">
...
// Simple manual JSON parser to avoid extra libraries
String getJsonValue(String json, String key) {
    int keyIdx = json.indexOf("\"" + key + "\"");
    if (keyIdx == -1) return "";
    int valStart = json.indexOf(":", keyIdx);
...
void handleConfigSave() {
    ...
    // Check if it's a JSON POST from the React frontend
    if (configServer.hasArg("plain")) {
        String body = configServer.arg("plain");
        Serial.println("Received JSON config: " + body);
        
        String s = getJsonValue(body, "ssid");
        String p = getJsonValue(body, "password");
        String b = getJsonValue(body, "server_ip"); // Dashboard uses server_ip
        String n = getJsonValue(body, "name");
    ...
    // Otherwise handle standard HTML form POST
    else {
...
void startConfigPortal() {
    ...
    // Generate unique AP name - Align with Nexora Dashboard
    ...
    configServer.on("/config", HTTP_POST, handleConfigSave); // Support both paths
    configServer.on("/config", HTTP_OPTIONS, handleOptions); // Support preflight
    configServer.begin();
...
bool initCamera() {
    ...
    config.xclk_freq_hz = CAMERA_XCLK_MHZ * 1000000;
    config.pixel_format = PIXFORMAT_JPEG;
    config.frame_size = FRAMESIZE_CIF;  // Lowering to CIF (400x296) for better memory compatibility
    config.jpeg_quality = 12;
    config.fb_count = 1;

    // FORCE INTERNAL DRAM ALLOCATION (Nuclear Fix for 0xffffffff)
    // Even if board claims to have PSRAM, internal DRAM is more reliable for 320x240
    config.frame_size = FRAMESIZE_QVGA; 
    config.fb_count = 1;
    config.jpeg_quality = 14; // Smaller frames
    config.fb_location = CAMERA_FB_IN_DRAM;
    
    // Optional PSRAM check - only use for extra buffers if found
    if(psramFound() && false){ // Hard-disabled for now to ensure internal RAM works first
        config.fb_count = 2;
        config.fb_location = CAMERA_FB_IN_PSRAM;
    }

    // Hardware Reset Sequence: Toggle PWDN to hard-reset the sensor
    ...
    if (err != ESP_OK) {
        Serial.printf("[Camera] Probe Failed: 0x%x. Try checking ribbon cable or lowering XCLK.\n", err);
        return false;
    }
...
void registerWithBackend() {
    ...
    // Safety check: Ensure port 8000 is present if not specified
    String url = backend_url;
    if (url.indexOf(":", 7) == -1) { // skip http://
        url += ":8000";
    }
    
    // Ensure the path is correct
    if (!url.endsWith("/api/cameras/register")) {
        if (!url.endsWith("/")) url += "/";
        url += "api/cameras/register";
    }
    
    Serial.println("[HTTP] Registering with corrected URL: " + url);
...
void setup() {
    ...
    // Init camera with hardware compatibility patch
    if (!initCamera()) {
        camera_ok = false;
        Serial.println("[CRITICAL] Camera Hardware Start Failed!");
        Serial.println("[INFO] Re-check the camera module connection. Will continue with web server only.");
    } else {
        camera_ok = true;
...
const unsigned long REG_INTERVAL = 30000; // Retry every 30 seconds

void loop() {
    // 1. Listen for Serial Commands (Fail-safe Reset)
    if (Serial.available()) {
        String cmd = Serial.readStringUntil('\n');
        cmd.trim();
        if (cmd.equalsIgnoreCase("reset")) {
            Serial.println("\n[System] !!! REMOTE RESET COMMAND RECEIVED !!!");
            Serial.println("[System] Wiping WiFi/IP configuration...");
            clearConfig();
            delay(1000);
            Serial.println("[System] Rebooting into Setup Mode...");
            delay(500);
            ESP.restart();
        }
    }
    ...
    if (configMode) {
        ...
        // Periodic registration retry (Heartbeat) - ONLY if camera hardware is working
        if (WiFi.status() == WL_CONNECTED && backend_url.length() > 0 && camera_ok) {
            if (millis() - lastRegAttempt > REG_INTERVAL || lastRegAttempt == 0) {
                lastRegAttempt = millis();
                Serial.println("[System] Periodic registration check...");
                registerWithBackend();
            }
        }
    }
