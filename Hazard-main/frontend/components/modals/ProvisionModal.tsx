...
export function ProvisionModal({ open, onOpenChange, onSuccess }: ProvisionModalProps) {
    ...
    const [config, setConfig] = useState({
        ssid: "",
        password: "",
        server_ip: "...", // Loading state
        name: "Nexora Cam " + Math.floor(Math.random() * 1000)
    });

    // Auto-detect Server IP on open
    useEffect(() => {
        fetch("/api/network/info")
            .then(res => res.json())
            .then(data => {
                if (data.local_ip) {
                    const fullIp = data.port ? `${data.local_ip}:${data.port}` : data.local_ip;
                    setConfig(prev => ({ ...prev, server_ip: fullIp }));
                }
            })
            .catch(err => console.error("Could not auto-detect IP:", err));
    }, []);
