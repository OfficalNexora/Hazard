...
# Conditional imports for Android (where cv2/serial might be missing)
try:
    ...
    print("[Server] Worker modules missing (Expected on Android). APIs will be limited.")
    # IDummy implementations to prevent NameError
    def get_vision_worker(): return None
    def get_control_worker(): return None
...
async def lifespan(app: FastAPI):
    ...
    # On Android, we disable heavy workers to save battery/resources
    if not os.environ.get("ANDROID_MODE"):
        try:
        ...
        print("[Server] ANDROID_MODE detected: Skipping worker initialization.")
...
# Modified path for Android structure
if os.environ.get("ANDROID_MODE"):
    # In Android, server.py is in backend/, assets are in ../assets
    FRONTEND_PATH = os.path.join(os.path.dirname(__file__), "..", "assets", "web")
    PUBLIC_PORTAL_PATH = os.path.join(os.path.dirname(__file__), "..", "assets", "public_web") # config if public portal exists
else:
